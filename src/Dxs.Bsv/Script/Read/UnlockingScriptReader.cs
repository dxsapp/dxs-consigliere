...
) : BaseScriptReader(bitcoinStreamReader, length, network)
{
    ...
    public DstasUnlockingSemantics Dstas { get; private set; }
    ...
    public static UnlockingScriptReader Read(BitcoinStreamReader bitcoinStreamReader, int expectedLength, Network network)
    {
        ...
        reader.ReadInternal();
        reader.TryFinalizeDstas();

        return reader;
    }

    private void TryFinalizeDstas()
    {
        if (!DstasUnlockingScriptParser.TryParse(_tokens, out var dstas))
            return;

        Dstas = dstas;
        DstasSpendingType = dstas.SpendingType;
    }
}
