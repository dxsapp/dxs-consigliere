...
| A2 hardening | `e4a534a` | 4 Low re-audit findings folded in one pass (see `audits/A2-hardening.md`): L1+L2 — `ForwardedHeadersTrustConfig` gives a working **string** env knob (`Consigliere__ForwardedHeaders__KnownNetworks/Proxies`) to restrict X-Forwarded-* trust (framework `IPAddress` lists don't bind from env); source comment + runbook corrected. L3 — `ProviderReachabilityCheck` caches its result 5s behind a single-flight gate (+ DI singleton) so an anonymous `/health/ready` flood collapses to one probe-set. L4 — `SecretsFileStore` chmods the tmp file 600 while EMPTY before writing secrets + locks the secrets dir to 700, closing the world-readable umask window. +9 backend test pins; build/verify/contract green. |
