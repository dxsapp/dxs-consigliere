...
| 5 | `broadcast-unification-wave` | indexer-write-path | done | W2, W4 | grep regression test `BroadcastUnificationGrepTests` (7 tests) enforces no legacy broadcast paths; real mainnet tx via single `Broadcast` method deferred to operator session (S7) per W2/W3/W4 pattern | single `BroadcastAsync` method on `IBroadcastService`; legacy `Broadcast(string)` / `Broadcast(Transaction)` + multi-provider helpers DELETED; `IBroadcastProvider` renamed to `IFeeRateProvider`; `BitcoindService.Broadcast` + `IBitailsRestApiClient.Broadcast` + `IWhatsOnChainRestApiClient.BroadcastAsync` DELETED. Implementation closed pending Codex A2 audit. | A2 pending |
...
- Wave 5 delivery (`broadcast-unification-wave`): implementation closed pending Codex A2 audit. S0-S6 delivered; S7 (live mainnet) operator-deferred per W2/W3/W4 pattern. Chain `0e87265` (package + A1 prompt) → `8a999d9` (S0: SubmitAsync → BroadcastAsync rename + IBroadcastServiceShapeTests pin) → this commit (S1 WalletHub + TransactionController unified entrypoint + S2 BroadcastService legacy method + helpers deletion + UnconfirmedTransactionsMonitor caller fix + S3 IBroadcastProvider → IFeeRateProvider rename + BitcoindService.Broadcast deletion + S4 Bitails + WoC client broadcast deletion + S5 BroadcastServiceTests legacy deletion + S6 BroadcastUnificationGrepTests 7-test regression suite + closeout). `IBroadcastService` ctor reduced from 10 deps → 3. Test counts: `Dxs.Bsv.Tests` 220/220, `Dxs.Consigliere.Tests` 424 passed (+6 from W4 close; +10 new W5 tests, -4 deleted legacy). Migration snippet for external wallet clients in closeout. Wave audit A2 pending.
