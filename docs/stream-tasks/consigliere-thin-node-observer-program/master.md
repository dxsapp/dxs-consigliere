...
| 4 | `observation-source-metrics-wave` | indexer-ingest-orchestration | done | W2, W3 | fixture-injected observations match counter values exactly; lag histogram bucket counts deterministic across all 6 buckets; per-source InvObserved + first-seen + only-saw counts pinned by `SourceMetricsEndToEndFixtureTests`; concurrent + race regression tests pin TryAdd / lock-aware eviction (A2 H1/H2); journal-writer hook uses source's own ObservedAt timestamp (A2 H3); aggregator round-trip via `ISnapshotPersistence` abstraction (A2 M3); `lastN` clamped via testable `ClampLastN` helper (A2 M2 + A2-followup N2) | `SourceMetricsAggregator` hosted service live; `AdminMetricsController` (`GET /api/admin/metrics/sources?lastN=N`) exposes latest + history; tracker hooked at `TxObservationJournalWriter` tail (both overloads, both pinned by regression tests); SPA page operator-deferred. **CLOSED**: A2 + A2-followup all folded | CLOSED — A2-followup APPROVE WITH CHANGES |
| 5 | `broadcast-unification-wave` | indexer-write-path | done | W2, W4 | grep regression test `BroadcastUnificationGrepTests` (7 tests) enforces no legacy broadcast paths; `BroadcastServiceBehaviorTests` (6 tests) covers persist/validate/announce/no-peer-non-terminal/duplicate paths; `BroadcastServiceProductionDiTests` (2 tests) pins real DI wiring through `AddRealtimeZoneServices` + `BroadcastServiceP2pWirer.Wire`; real mainnet tx deferred to operator session per W2/W3/W4 pattern | single `BroadcastAsync` method on `IBroadcastService`; legacy `Broadcast(string)` / `Broadcast(Transaction)` + multi-provider helpers DELETED; `IBroadcastProvider` renamed to `IFeeRateProvider`; `BitcoindService.Broadcast` + `IBitailsRestApiClient.Broadcast` + `IWhatsOnChainRestApiClient.BroadcastAsync` DELETED. Ctor 10 → 2 deps (`IBitcoindService` + `ILogger`). Property-injected slots interface-typed (`IBroadcastPolicyValidator` + `IOutgoingTransactionRepository` + `ITxAnnouncer`). No-ready-peer path leaves tx in `Dispatching` (Core Rule §2 H1 fix). **CLOSED**: A2 + A2-followup all folded | CLOSED — A2-followup APPROVE WITH CHANGES |
| 6 | `production-ops-wave` | indexer-ingest-orchestration | done | W2, W4, W5 | alert fires when pool drops below threshold in fixture (`P2pAlertPoller_PoolBelowThreshold_FiresEvent` + 5 sibling done-when fixtures green); peer rotation evicts low-scoring peer in fixture (`PeerRotationPlanner_EvictsLowestScoringPeer`); runbook reviewed (`docs/platform-api/thin-node-prod-runbook.md`) | peer scoring + planner-driven rotation live; 4-rule alert poller wired; `GET /api/admin/p2p/alerts` admin endpoint; runbook + W5 changeout doc published. **CLOSED** with program-level closeout at `evidence/closeout.md` | A1 (3 passes APPROVE) + S0 + S1+S2 slice-audits all cleared |
...
- Wave 4 delivery (`observation-source-metrics-wave`): **CLOSED 2026-05-18**. S0-S7 delivered; S8 (SPA page) operator-deferred. Full audit chain folded: A2 → A2-followup APPROVE WITH CHANGES. Chain `26a823f` (package + A1 prompt) → `9fafd5a` (S0+S1+S2; 27 unit tests) → `7cc1385` (S3+S4+S5+S6+S7; 18 more tests; closeout draft + A2 prompt) → `bca6f7c` (A2 revision: H1 TryAdd-pattern + concurrent test, H2 lock-aware eviction + race test, H3 source-observed-at timestamp, M1 immutable bucket bounds, M2 lastN clamp, M3 ISnapshotPersistence abstraction, L1 distinct-window DI flow-through) → this commit (A2-followup: N1 TxMessage overload regression tests, N2 ClampLastN extracted to internal helper + direct theory-test; `InternalsVisibleTo` added). Test counts: `Dxs.Bsv.Tests` 220/220, `Dxs.Consigliere.Tests` 418 passed (+69 from W3 close), metrics-filtered 71/71. 3 pre-existing Raven embedded-runtime baseline failures unchanged.
- Wave 5 delivery (`broadcast-unification-wave`): **CLOSED 2026-05-18**. S0-S6 delivered; S7 (live mainnet) operator-deferred. Full audit chain folded: A2 → A2-followup APPROVE WITH CHANGES. Chain `0e87265` (package + A1 prompt) → `8a999d9` (S0 rename + shape pin) → `e4ab238` (S1-S6 + closeout draft + A2 prompt) → `1592fdb` (A2 revision: H1 no-ready-peer non-terminal fix + M1 5 behavioral tests via IBroadcastPolicyValidator/IOutgoingTransactionRepository abstractions + M2 production DI graph test + L1 IDocumentStore dep removed + L2 BroadcastResponseDto deleted + L3 IsActiveOrAccepted helper + L4 BroadcastTxRequest moved + L5 closeout S2 row corrected) → this commit (A2-followup: M1 duplicate-receipt test added + M2 class-comment fixed + N1 master.md ctor drift corrected + N2 nullable annotations fixed). Final ctor: 10 → 2 deps (`IBitcoindService` + `ILogger`). Test counts: `Dxs.Bsv.Tests` 220/220, `Dxs.Consigliere.Tests` 433 passed (+9 from W4 close: +10 new W5 tests including duplicate + M2, -4 deleted legacy, +3 ripple) + 24 Skipped + 3 pre-existing baseline Raven failures unchanged. Migration snippet for external wallet clients in closeout.
