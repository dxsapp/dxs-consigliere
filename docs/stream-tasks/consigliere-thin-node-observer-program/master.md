...
| 3 | `reorg-handling-wave` | indexer-state-and-storage | done | W1, W2 | 1/2/N-deep fork tests; deep-reorg-beyond-window asserts degraded state; mismatched-body provider rejected (N/A — S2 deferred); idempotent disconnect events | reorg detector live; `Reorged` state produced via existing rebuilder; `OnReorg` event fired; orphaned tx re-announced via `TxRelayCoordinator.AnnounceAsync`. Implementation closed pending Codex audit A1 + A2 | A1 pending |
...
- Wave 3 delivery (`reorg-handling-wave`): implementation closed pending Codex pre-execution audit A1 + post-execution audit A2. S0 + S1 + S3 + S4 + S5 + S6 delivered; S2 deferred (P2P block-body fetch is GB-scale, projection `BlockHash` index used instead — see closeout "Scope deviations"); S7 operator-deferred (live mainnet). Chain `92ead7d` (package draft) → `091299e` (S0 + A1-prompt) → `bfac702` (S1 ReorgDetector) → `9c64818` (S3 ReorgPipeline + S4 OrphanedTxRebroadcaster + S5 DI wiring + W3_SingletonGraph_Resolves + 5-deep depth pin) → this commit (evidence/closeout + program ledger flip). Test counts: `Dxs.Bsv.Tests` 218 (+11), `Dxs.Consigliere.Tests` 340 passed (+25). Wave audit A1 pre-execution prompt landed in `audits/wave3-audit-A1-prompt.md` for user-driven Codex review.
