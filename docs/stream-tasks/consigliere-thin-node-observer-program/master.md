...
| 3 | `reorg-handling-wave` | indexer-state-and-storage | done | W1, W2 | 1/2/N-deep fork tests; deep-reorg-beyond-window asserts degraded state; mismatched-body provider rejected (N/A — S2 deferred); idempotent disconnect events; low-difficulty fork rejected by chainwork (A2 C1); fork tip promoted to active (A2 C2) | reorg detector live; `Reorged` state produced via existing rebuilder; `OnReorg` event fired AFTER rebuilder runs (Core Rule §9 enforced strictly); active tip swapped via `HeadersChain.PromoteFork`; orphaned tx re-announced via `TxRelayCoordinator.AnnounceAsync` with explicit coinbase exclusion. A2 closed (MAJOR REVISION REQUIRED → folded); A1 prompt staged for user-Codex pass; A2-followup pending | A2 closed (10/10 findings folded) |
...
- Wave 3 delivery (`reorg-handling-wave`): A2 closed (10/10 findings folded). S0 + S1 + S3 + S4 + S5 + S6 delivered; S2 deferred (P2P block-body fetch is GB-scale, projection `BlockHash` index used instead — see closeout "Scope deviations"); S7 operator-deferred (live mainnet). Chain `92ead7d` (package draft) → `091299e` (S0 + A1-prompt) → `bfac702` (S1 ReorgDetector) → `9c64818` (S3 ReorgPipeline + S4 OrphanedTxRebroadcaster + S5 DI wiring + W3_SingletonGraph_Resolves + 5-deep depth pin) → `1fab0ef` (closeout draft) → `e9d2691` (A2 prompt) → this commit (A2 revision: C1 chainwork comparator + C2 PromoteFork + H1 IProjectionRebuilder injection + H2 ICoinbaseProbe + H3 SemaphoreSlim gate + M1 transactional pipeline failure + M3 strict-sequence ordering pin + L1 expanded DI assertions). Test counts after A2 revision: `Dxs.Bsv.Tests` 220 (+13 from pre-W3 baseline), `Dxs.Consigliere.Tests` 341 passed (+26 from pre-W3 baseline). Wave audit A2-followup pending; A1 pre-execution prompt is also staged in `audits/wave3-audit-A1-prompt.md`.
