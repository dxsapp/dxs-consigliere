...
| S8 | system (P2P Pool) | todo | S3 | peers `DataGrid` 60-70% with ScoreBar + per-component MiniBars; subnet/24 donut from `@mui/x-charts` 30-40%; **S8 owns the score-gradient WCAG AA contrast check in both palettes** (A1 M10); **route-level lazy boundary REQUIRED** so DataGrid + X Charts land in a per-route chunk, not the shell (S3-audit M6 — shell already at 198 KB / 200 KB budget) | live peer telemetry; donut updates on poll; matches `frames-p2p.jsx`; gradient contrast report committed; per-route chunk reported separately from shell budget | wave-A1 |
